...
def register_and_login(client, headers):
    ...
    try:
        ...
        for target in hosts_to_try:
            ...
            for config in auth_configs:
                try:
                    r = redis.Redis(host=target["host"], port=target["port"], db=0, decode_responses=True, socket_timeout=2, **config)
                    # Key template from constants.py: "email_otp_{receiver}_{intent}"
                    otp_key = CacheKeyTemplates.OTP_EMAIL.format(receiver=email, intent=Intents.REGISTRATION)
                    otp = r.get(otp_key)
                    if otp:
                        break
                    # Even if no OTP, if ping works, we might just be looking at the wrong key
                    if r.ping():
                        print(f"DEBUG: Connected to Redis at {target['host']}:{target['port']} with config {config}, but OTP not found for {email}")
        ...
        if not otp:
            print(f"DEBUG: OTP not found in Redis for {email} after trying {hosts_to_try}")
            return None, None
    ...
    with client.post(
        ...
        if resp.status_code == 200:
            auth_token = resp.json().get("data", {}).get("token") # Note: login views shows 'token' in data_dict
