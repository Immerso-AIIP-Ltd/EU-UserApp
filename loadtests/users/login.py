...
class LoginUser(HttpUser):
    ...
    def login_user(self):
        ...
        with self.client.post("/user/v1/user/login", json=payload, headers=self.headers, catch_response=True) as response:
             ...
             if response.status_code in [200, 400, 401, 404]:
                 ...
                 response.failure(f"Login unexpected error: {response.status_code} - {response.text}")
    ...
    def forgot_password(self):
        email = f"loadtest_{''.join(random.choices(string.ascii_lowercase, k=5))}@example.com"
        payload = {"email": email}
        with self.client.post("/user/v1/user/forgot_password", json=payload, headers=self.headers, catch_response=True) as response:
