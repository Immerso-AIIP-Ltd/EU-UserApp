...
class DeviceTokenRedisService:
    ...
    async def store_device_token_in_redis(self, device: dict, user_uuid: str):
        """
        ...
        try:
            ...
            if not push_token:
                logger.debug(f"No push token for device {device_id}, skipping Redis store")
            ...
            error_message = f"Error storing device token in Redis: {str(e)}"
            logger.error(error_message)
            return {'success': False, 'error': str(e)}

    async def remove_device_token_from_redis(self, device: dict, user_uuid: str):
        """
        ...
        try:
            ...
            error_message = f"Error removing device token from Redis: {str(e)}"
            logger.error(error_message)
            return {'success': False, 'error': str(e)}

    async def _update_user_device_list_in_redis(self, user_uuid: str):
        ...
        try:
            ...
            if active_devices:
                await self.redis_client.setex(user_devices_key, self.ttl, json.dumps(active_devices))
                logger.debug(f"Updated user device list in Redis: {len(active_devices)} devices")
            else:
                await self.redis_client.delete(user_devices_key)
                logger.debug(f"Removed user device list from Redis (no active devices)")

        except Exception as e:
            logger.error(f"Failed to update user device list in Redis for user {user_uuid}: {str(e)}")

    async def get_user_device_tokens(self, user_uuid: str):
        ...
        try:
            ...
            logger.error(f"Failed to get device tokens from Redis for user {user_uuid}: {str(e)}")
            return []
