...
async def verify_otp_register(
    ...
    redis_config = {
        "cluster_nodes": settings.redis_cluster_nodes if hasattr(settings, 'redis_cluster_nodes') else None,
    ...
    if push_token and cache:
        try:
            ...
            logger.info(
                ...
                f"  Push Token (first 50 chars): {push_token[:50] if push_token else 'None'}..."
            )
            ...
            logger.info(
                f"[REDIS SUCCESS] Device token successfully stored in Redis for user {user_id}"
            )
