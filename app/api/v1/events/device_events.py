...
def sync_device_token_to_redis(device_dict: dict[str, Any], user_uuid: str) -> None:
    ...
    try:
        ...
        from app.cache.dependencies import (  # type: ignore[attr-defined]
            get_sync_redis_client,
        )

        redis_client = get_sync_redis_client()  # type: ignore[attr-defined]
        if not redis_client:
        ...
        if result.get("success"):
            logger.info(
                f"Device token {action} in Redis for device "
                f"{device_dict.get('device_id')}",
            )
        else:
            logger.warning(
    ...
    try:
        ...
        if target.push_token and target.user_id:
            ...
            if not target.push_token:
                logger.debug(
                    f"Device {target.serial_number} has no push_token, "
                    "skipping Redis sync",
                )
            if not target.user_id:
                logger.debug(
                    f"Device {target.serial_number} not linked to user, "
                    "skipping Redis sync",
                )

    except Exception as e:
        logger.error(f"Error in device_after_insert for {target.serial_number}: {e!s}")


@event.listens_for(Device, "after_update")
def device_after_update(
    mapper: Any,
    connection: Any,
    target: Device,
) -> None:
    ...
    try:
        ...
        logger.error(f"Error in device_after_update for {target.serial_number}: {e!s}")


@event.listens_for(Device, "after_delete")
def device_after_delete(
    mapper: Any,
    connection: Any,
    target: Device,
) -> None:
    ...
    try:
        ...
        if target.push_token and target.user_id:
            ...
            if redis_client:
                ...
                logger.info(
                    f"Device deleted and token removed from Redis: "
                    f"{target.serial_number}",
                )
        else:
            logger.debug(
        ...
        logger.error(f"Error in device_after_delete for {target.serial_number}: {e!s}")
