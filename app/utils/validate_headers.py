...
class CommonHeadersWithoutXDeviceId(BaseModel):
    """Pydantic model for common request headers without authentication."""
...
async def validate_headers_without_x_device_id(
    ...
    if not all([x_api_client, x_platform, x_country, x_app_version]):
        raise MissingHeadersError(detail=ErrorMessages.MISSING_HEADERS_DETAILS)

    return CommonHeadersWithoutXDeviceId.model_validate(
        {
            "x-api-client": x_api_client,
            "x-platform": x_platform,
            "x-country": x_country,
